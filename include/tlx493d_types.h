...
/**
 * @brief Used to store name, access mode, address, mask and offset of registers.
 */
...
typedef bool (*TLx493D_TransferFuncPtr)(
    TLx493D_t* sensor, uint8_t* tx_buffer, uint8_t tx_len, uint8_t* rx_buffer, uint8_t rx_len
);

/**
 * @brief Used to stores the parameter of the SPI interface.
 */
...
/**
 * @brief Used to stores the parameter of the I2C interface.
 */
...
/**
 * @brief Used to stores the function pointers which are needed for
 * the initialization and the transfer of data, for each communication interface.
 */
...
/**
 * @brief Used to store the necessary parameters for both interface (I2C, SPI).
 */
...
/**
 * @brief Used to store both communication interface objects.
 */
...
/**
 * @brief Used to contain all the important parameters to properly use the communication interface.
 */
...
typedef void (*TLx493D_CalculateRawMagneticFieldAndTemperatureFuncPtr)(
    TLx493D_t*, int16_t*, int16_t*, int16_t*, int16_t*
);
...
typedef bool (*TLx493D_SetWakeUpThresholdsAsIntegerFuncPtr)(
    TLx493D_t*, int16_t, int16_t, int16_t, int16_t, int16_t, int16_t);
...
typedef void (*TLx493D_CalculateRawMagneticFieldAtTemperatureFuncPtr)(
    const TLx493D_t*, int16_t, TLx493D_SensitivityType_t, double, double, double, int16_t*, int16_t*, int16_t*
);
...
/**
 * Functions used to refer to sensor specific functions by a common name.
 * These functions are not part of the common user interface.
 */
...
/**
 * @brief Stores all relevant information for a particular sensor.
 */
