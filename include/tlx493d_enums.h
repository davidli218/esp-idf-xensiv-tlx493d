...
/**
 * @brief All the supported sensor types of the library.
 */
typedef enum {
    TLx493D_A1B6_e = 0,
    TLx493D_A2B6_e,
...
/**
 * @brief The supported communication interfaces of the different sensors.
 */
typedef enum {
    TLx493D_I2C_e = 0,
    TLx493D_SPI_e,
...
/**
 * @brief The access modes of the different registers inside the sensors.
 */
typedef enum {
    TLx493D_READ_MODE_e = 0,
    TLx493D_WRITE_MODE_e,
    TLx493D_READ_WRITE_MODE_e,
} TLx493D_RegisterAccessModeType_t;

/**
 * @brief The possible I2C addresses of the different sensors.
 */
typedef enum {
...
/**
 * @brief The different measurement modes of the sensors.
 *
 * You have to make sure that you are using a mode which is
 * available for your used sensor family (1st Gen, 2nd Gen, etc.).
 */
typedef enum {
...
/**
 * @brief The different trigger modes for the sensors.
 *
 * You have to make sure that you are using a mode which is
 * available for your used sensor family (1st Gen, 2nd Gen, etc.).
 */
...
/**
 * @brief The different range settings for the sensors.
 */
typedef enum {
...
/**
 * @brief The different bits which define the range of the used sensor.
 *
 * They are different depending on the used sensor type.
 * The `TLx493D_SensitivityType_t` depends on the available `TLx493D_AvailableSensitivityType_t`.
 */
typedef enum {
    TLx493D_HAS_X1_e = 0,
    TLx493D_HAS_X2_e,
...
/**
 * @brief The different power modes of the sensors.
 *
 * The available ones are depend on the sensor variant that you are using.
 */
typedef enum {
...
/**
 * @brief The different update rates of the sensors.
 *
 * Depending on which sensor variant you're using you can choose different update rates.
 */
typedef enum {
    /** Generation 2 */
