...
#include "tlx493d_g2_common.h"
#include "tlx493d_g2_common_defines.h"


static const char* TAG = "tlx493d_g2_common";
...
bool tlx493d_gen_2_setOneConfigBitfield(
    TLx493D_t* sensor, const uint8_t bfBF, const uint8_t cpBF, const uint8_t bfVal
) {
    tlx493d_common_setBitfield(sensor, bfBF, bfVal);
    tlx493d_common_setBitfield(
        sensor, cpBF, sensor->functions->calculateConfigurationParity(sensor)
    );
...
bool tlx493d_gen_2_setTwoConfigBitfields(
    TLx493D_t* sensor,
    const uint8_t firstBF, const uint8_t secondBF, const uint8_t cpBF,
    const uint8_t firstVal, const uint8_t secondVal
) {
...
void tlx493d_gen_2_calculateRawMagneticField(
    ...
    tlx493d_common_calculateRawMagneticField(
        sensor, bxMSBBF, bxLSBBF, byMSBBF, byLSBBF, bzMSBBF, bzLSBBF, x, y, z
    );
}

void tlx493d_gen_2_calculateTemperature(
    const TLx493D_t* sensor, const uint8_t tempMSBBF, const uint8_t tempLSBBF, double* temperature
) {
    int16_t value = 0;
...
void tlx493d_gen_2_calculateMagneticField(
    ...
    const double scaledSensitivity =
        GEN_2_FULL_RANGE_FIELD_SENSITIVITY * sensor->functions->getSensitivityScaleFactor(sensor);
...
bool tlx493d_gen_2_setMeasurement(
    TLx493D_t* sensor, const uint8_t dtBF, const uint8_t amBF, const uint8_t cpBF, const TLx493D_MeasurementType_t val
) {
    uint8_t dt = 0;
    uint8_t am = 0;
...
bool tlx493d_gen_2_setTrigger(
    TLx493D_t* sensor, const uint8_t trigBF, const uint8_t cpBF, const TLx493D_TriggerType_t val
) {
    uint8_t trig = 0;
...
bool tlx493d_gen_2_setSensitivity(
    TLx493D_t* sensor, const TLx493D_AvailableSensitivityType_t availSens,
    const uint8_t x2BF, const uint8_t x4BF, const uint8_t cpBF, const TLx493D_SensitivityType_t val
) {
    (void)x4BF;

    switch (availSens) {
        case TLx493D_HAS_X1_e:
            return val == TLx493D_FULL_RANGE_e;

        case TLx493D_HAS_X2_e:
            return val == TLx493D_EXTRA_SHORT_RANGE_e
                   ? false
                   : tlx493d_gen_2_setOneConfigBitfield(sensor, x2BF, cpBF, val == TLx493D_FULL_RANGE_e ? 0 : 1);

        default:
            tlx493d_errorSelectionNotSupportedForSensorType(
                sensor, availSens, "TLx493D_AvailableSensitivityType_t"
            );
            return false;
    }
}

bool tlx493d_gen_2_setDefaultConfig(
    TLx493D_t* sensor, const uint8_t cpBF, const uint8_t caBF, const uint8_t intBF
) {
    ...
    if (sensor->functions->enable1ByteReadMode(sensor)) {
        if (!sensor->functions->readRegisters(sensor)) {
            ESP_LOGE(TAG, "readRegisters failed!");
        }

        if (tlx493d_common_returnBitfield(sensor, cpBF) == 0x01U
            && !sensor->functions->hasValidConfigurationParity(sensor)) {
...
bool tlx493d_gen_2_setIICAddress(
    TLx493D_t* sensor, const uint8_t iicadrBF, const uint8_t fpBF, const TLx493D_IICAddressType_t addr
) {
    ...
    const bool b = tlx493d_common_writeRegister(sensor, fpBF);
    tlx493d_common_setIICAddress(sensor, deviceAddress);

    return b;
}

bool tlx493d_gen_2_set1ByteReadMode(
    TLx493D_t* sensor, const uint8_t prBF, const uint8_t fpBF, const uint8_t prdBF, const uint8_t pr
) {
 ...
 */
bool tlx493d_gen_2_setCollisionAvoidance(
    TLx493D_t* sensor, const uint8_t caBF, const uint8_t fpBF, const uint8_t ca
) {
...
bool tlx493d_gen_2_setInterrupt(TLx493D_t* sensor, const uint8_t intBF, const uint8_t fpBF, const uint8_t irq) {
...
bool tlx493d_gen_2_setPowerMode(
    TLx493D_t* sensor, const uint8_t modeBF, const uint8_t fpBF, const TLx493D_PowerModeType_t val
) {
    const uint8_t mod1 = sensor->regDef[fpBF].address;
    uint8_t mode = 0;
    ...
    uint8_t buf[4] = {
        mod1,
        sensor->regMap[mod1],
        sensor->regMap[mod1 + 1U],
        /* reserved register must have been read once in setDefaultConfig to get factory settings. */
...
bool tlx493d_gen_2_setUpdateRateFastSlow(
    TLx493D_t* sensor, const uint8_t fpBF, const uint8_t prdBF, const TLx493D_UpdateRateType_t val
) {
    const uint8_t mod1 = sensor->regDef[fpBF].address;
    uint8_t rate = 0;
...
bool tlx493d_gen_2_setUpdateRate(
    TLx493D_t* sensor, const uint8_t fpBF, const uint8_t prdBF, const TLx493D_UpdateRateType_t val
) {
    const uint8_t mod1 = sensor->regDef[fpBF].address;
    uint8_t rate = 0;
...
bool tlx493d_gen_2_hasValidData(
    const TLx493D_t* sensor, const uint8_t modeBF, const uint8_t pd3BF, const uint8_t pd0BF
) {
    return sensor->functions->hasValidBusParity(sensor)
           && sensor->functions->hasValidTBit(sensor)
           && (tlx493d_common_returnBitfield(sensor, modeBF) == 0b11U
               ? true
               : tlx493d_common_returnBitfield(sensor, pd3BF) == 1U
                 && tlx493d_common_returnBitfield(sensor, pd0BF) == 1U);
}
...
bool tlx493d_gen_2_isWakeUpEnabled(const TLx493D_t* sensor, const uint8_t waBF) {
...
bool tlx493d_gen_2_enableWakeUpMode(
    TLx493D_t* sensor, const uint8_t tstBF, const uint8_t wuBF, const uint8_t cpbBF
) {
    const bool isTestMode = tlx493d_common_returnBitfield(sensor, tstBF) != 0U;
...
bool tlx493d_gen_2_disableWakeUpMode(TLx493D_t* sensor, const uint8_t wuBF, const uint8_t cpbBF) {
...
bool tlx493d_gen_2_setThreshold(
    const TLx493D_t* sensor, const uint8_t msbsBF, const uint8_t lsbsBF, const int16_t threshold12Bits
) {
    TLx493D_Register_t const* msbs = &sensor->regDef[msbsBF];
    TLx493D_Register_t const* lsbs = &sensor->regDef[lsbsBF];

    const int16_t thresh11Bits = (int16_t)(threshold12Bits >> 1U);

    const uint8_t lower = thresh11Bits & lsbs->mask >> lsbs->offset;
    const uint8_t upper = thresh11Bits >> lsbs->numBits & msbs->mask >> msbs->offset;
...
bool tlx493d_gen_2_setWakeUpThresholdsAsInteger(
    TLx493D_t* sensor, const uint8_t cpBF,
    const uint8_t xlMSBBF, const uint8_t xlLSBBF, const uint8_t xhMSBBF, const uint8_t xhLSBBF,
    const uint8_t ylMSBBF, const uint8_t ylLSBBF, const uint8_t yhMSBBF, const uint8_t yhLSBBF,
    const uint8_t zlMSBBF, const uint8_t zlLSBBF, const uint8_t zhMSBBF, const uint8_t zhLSBBF,
    const int16_t xlTh, const int16_t xhTh,
    const int16_t ylTh, const int16_t yhTh,
    const int16_t zlTh, const int16_t zhTh
) {
...
bool tlx493d_gen_2_setWakeUpThresholds(
    ...
    const uint8_t ylMSBBF, const uint8_t ylLSBBF, const uint8_t yhMSBBF, const uint8_t yhLSBBF,
    const uint8_t zlMSBBF, const uint8_t zlLSBBF, const uint8_t zhMSBBF, const uint8_t zhLSBBF,
    const TLx493D_AvailableSensitivityType_t availSens, const uint8_t x2BF, const uint8_t x4BF,
    const double temperature,
    const double xLow, const double xHigh,
    const double yLow, const double yHigh,
    const double zLow, const double zHigh
) {
    int16_t xlTh, xhTh, ylTh, yhTh, zlTh, zhTh;
    int16_t tr;

    tlx493d_gen_2_convertTemperatureToRaw(sensor, temperature, &tr);

    const TLx493D_SensitivityType_t sens = tlx493d_gen_2_getSensitivityType(sensor, availSens, x2BF, x4BF);

    sensor->functions->calculateRawMagneticFieldAtTemperature(
        sensor, tr, sens, xLow, yLow, zLow, &xlTh, &ylTh, &zlTh
    );
    sensor->functions->calculateRawMagneticFieldAtTemperature(
        sensor, tr, sens, xHigh, yHigh, zHigh, &xhTh, &yhTh, &zhTh
    );

    return tlx493d_gen_2_setWakeUpThresholdsAsInteger(
        sensor, cpBF,
...
uint8_t tlx493d_gen_2_calculateFuseParity(const TLx493D_t* sensor, const uint8_t fpBF, const uint8_t prdBF) {
...
uint8_t tlx493d_gen_2_calculateBusParity(const TLx493D_t* sensor, const uint8_t to) {
...
uint8_t tlx493d_gen_2_calculateConfigurationParity(const TLx493D_t* sensor, const uint8_t cpBF) {
    const TLx493D_Register_t* cp = &sensor->regDef[cpBF];
    const uint8_t parity = tlx493d_common_calculateParity(sensor->regMap[cp->address] & (uint8_t)~cp->mask);
    return tlx493d_common_getEvenParity(parity);
}

uint8_t tlx493d_gen_2_calculateConfigurationParityWakeUp(const TLx493D_t* sensor, const uint8_t cpBF) {
    const TLx493D_Register_t* cp = &sensor->regDef[cpBF];
    uint8_t parity = sensor->regMap[0x07U];
...
bool tlx493d_gen_2_hasValidFuseParity(const TLx493D_t* sensor, const uint8_t ffBF) {
    return tlx493d_common_returnBitfield(sensor, ffBF) != 0U;
}

bool tlx493d_gen_2_hasValidBusParity(const TLx493D_t* sensor, const uint8_t pBF) {
    return sensor->functions->calculateBusParity(sensor) == tlx493d_common_returnBitfield(sensor, pBF);
}

bool tlx493d_gen_2_hasValidConfigurationParity(const TLx493D_t* sensor, const uint8_t cpBF) {
    return tlx493d_common_returnBitfield(sensor, cpBF) != 0U;
}

bool tlx493d_gen_2_hasValidIICadr(const TLx493D_t* sensor, const uint8_t idBF, const uint8_t iicAdrBF) {
    return tlx493d_common_returnBitfield(sensor, idBF) == tlx493d_common_returnBitfield(sensor, iicAdrBF);
}

bool tlx493d_gen_2_hasValidTBit(const TLx493D_t* sensor, const uint8_t tBF) {
    return tlx493d_common_returnBitfield(sensor, tBF) == 0U;
}

uint8_t tlx493d_gen_2_selectIICAddress(const TLx493D_t* sensor, const TLx493D_IICAddressType_t addr) {
    switch (addr) {
...
double tlx493d_gen_2_getSensitivityScaleFactor(
    const TLx493D_t* sensor, const TLx493D_AvailableSensitivityType_t availSens, const uint8_t x2BF, const uint8_t x4BF
) {
    switch (availSens) {
        ...
        case TLx493D_HAS_X4_e: {
            const bool x2IsNotSet = tlx493d_common_returnBitfield(sensor, x2BF) == 0U;
            const bool x4IsNotSet = tlx493d_common_returnBitfield(sensor, x4BF) == 0U;
...
static double tlx493d_gen_2_getSensitivityScaleFactorFromSensitivity(const TLx493D_SensitivityType_t sens) {
    return sens == TLx493D_FULL_RANGE_e ? 1.0 : sens == TLx493D_SHORT_RANGE_e ? 2.0 : 4.0;
}

void tlx493d_gen_2_calculateRawMagneticFieldAtTemperature(
    const TLx493D_t* sensor, const int16_t rawTemp, const TLx493D_SensitivityType_t sens,
    const double xInmT, const double yInmT, const double zInmT, int16_t* x, int16_t* y, int16_t* z
) {
    (void)sensor;
    (void)rawTemp;

    const double scaledSensitivity =
        GEN_2_FULL_RANGE_FIELD_SENSITIVITY * tlx493d_gen_2_getSensitivityScaleFactorFromSensitivity(sens);
...
void tlx493d_gen_2_convertTemperatureToRaw(
    const TLx493D_t* sensor, const double temperature, int16_t* rawTemperature
) {
    (void)sensor;

    *rawTemperature = (int16_t)lround(
        ((temperature - GEN_2_TEMP_REF) / GEN_2_TEMP_RESOLUTION + GEN_2_TEMP_OFFSET) / 4.0
    );
}

TLx493D_SensitivityType_t tlx493d_gen_2_getSensitivityType(
    const TLx493D_t* sensor,
    const TLx493D_AvailableSensitivityType_t availSens, const uint8_t x2BF, const uint8_t x4BF
) {
    switch (availSens) {
        ...
        case TLx493D_HAS_X2_e: return tlx493d_common_returnBitfield(sensor, x2BF) == 0U
